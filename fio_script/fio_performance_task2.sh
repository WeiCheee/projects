...
#-------------------------------------------------------------------------------------------------------------------------
echo "task1"
time fio -output=/home/hsu/Desktop/fio_result/4k_randwrite_32_63G_libaio_1 -name=test -filename=/dev/sdd -ioengine=libaio -direct=1 -thread=1 -blocksize=4k -rw=randwrite -iodepth=32 -numjobs=1 -runtime=1200 -time_based=1
sleep 10
echo "End task1"
...
function run(){
	...
	do 
		...
		time fio -output=/home/hsu/Desktop/fio_result/${blockarray[a]}_${rwarray[0]}_${dparray[0]}_${sizearray[0]}_${enginearray[0]}_${countarray[0]} -name=test -filename=/dev/sdd -ioengine=${enginearray[0]} -direct=1 -thread=1 -blocksize=${blockarray[a]} -rw=${rwarray[0]} -iodepth=${dparray[0]} -numjobs=1 -runtime=60 -time_based=1						
		sleep 10
		time fio -output=/home/hsu/Desktop/fio_result/4k_randwrite_32_63G_libaio_withBlockarray${blockarray[a]}_${countarray[1]} -name=test -filename=/dev/sdd -ioengine=libaio -direct=1 -thread=1 -blocksize=4k -rw=randwrite -iodepth=32 -numjobs=1 -runtime=600 -time_based=1
