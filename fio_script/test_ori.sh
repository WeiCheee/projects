...
function script(){
    ...
    fio -output=/home/hsu/Desktop/fio_result/${block}_${rw}_${runtime} -name=test -filename=/dev/sdd -ioengine=libaio -direct=1 -thread=1 -blocksize=${block} -rw=${rw} -iodepth=32 -numjobs=1 -runtime=${runtime} -time_based=1
}
...
sleep 10
# step 2 ----------------------------------------------
fio -output=/home/hsu/Desktop/fio_result/4M_randwrite_step2 -name=test -filename=/dev/sdd -ioengine=libaio -direct=1 -thread=1 -blocksize=4k -rw=randwrite -iodepth=32 -numjobs=1 -runtime=7200 -time_based=1
sleep 10
# step 3 ----------------------------------------------
...
do
    script "1024k" "randwrite" $runtime
done
sleep 10
# step 4 ----------------------------------------------
fio -output=/home/hsu/Desktop/fio_result/1M_write_step4 -name=test -filename=/dev/sdd -ioengine=libaio -direct=1 -thread=1 -blocksize=1024k -rw=write -iodepth=32 -numjobs=1 -size=63G
fio -output=/home/hsu/Desktop/fio_result/1M_write_step4_2 -name=test -filename=/dev/sdd -ioengine=libaio -direct=1 -thread=1 -blocksize=1024k -rw=write -iodepth=32 -numjobs=1 -size=63G
sleep 10
# # step 5 ----------------------------------------------
for runtime in {8000,240}
do
